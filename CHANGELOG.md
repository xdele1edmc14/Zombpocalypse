...
- Added `%xapocalypse_bloodmoon_days_left%` for the whole number of in-game days until the next natural Blood Moon. It returns `0` while a Blood Moon is active or due that day.
...
- Added `%xapocalypse_current_scent%` for the active player's live scent value, returning `0` when no player context or scent is available.
...
- Updated all current release badges, help text, installation examples, artifact names, and resource headers to version 1.5.0.
